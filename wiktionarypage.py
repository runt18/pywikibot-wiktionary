...
The objects can output the content again in Wiktionary format by means of the
wikiWrap methods
...
still rather limited, only 3 parts of speech, only 2 different Wiktionary
output formats, only langnames matrix for about 8 languages. One of the things
on the todo list is to harvest the content of this matrix dictionary from the
various Wiktionary projects. GerardM put them all on line in templates already.
"""
...
class WiktionaryPage:
    ...
    def __init__(self, wikilang, term):
        ...
        self.term = term
        # entries is a dictionary of entry objects indexed by entrylang
        self.entries = {}
        self.sortedentries = []
        self.interwikilinks = []
        self.categories = []
...
##        self.entries.setdefault(entry.entrylang, []).append(entry)
    ...
    def sortEntries(self):
        ...
        if not self.entries == {}:
            ...
            self.sortedentries.sort(sortonlanguagename.sortonlanguagename(
                structs.langnames[self.wikilang]))
...
##        print self.interwikilinks
    ...
    def parseWikiPage(self, content):
        ...
        It returns a list of dictionaries. Each dictionary contains a header
        object and the textual content found under that header. Only relevant
        content is stored. Empty lines and lines to create tables for
        presentation to the user are taken out.
...
##            print line
            ...
            line = line.replace('\n', '').strip()
            # Let's start by looking for general stuff, that provides
            ...
            if '|' not in line:
                ...
                if bracketspos != -1 and colonpos != -1 and \
                   bracketspos < colonpos:
                    ...
                    continue
            # store empty lines literally, this is necessary for the blocks we
            # don't parse and will return literally
            if len(line) < 2:
                templist.append(line)
                continue
##            print 'line0:', line[0], 'line-2:', line[-2],'|',
##            print 'stripped line-2', print line.rstrip()[-2]
            if line.strip()[0] == '=' and line.rstrip()[-2] == '=' or \
               '{{-' in line and '-}}' in line:
                # When a new header is encountered, it is necessary to store
                # the information encountered under the previous header.
...
##                print "Header parsed:", aheader.level, aheader.header,
##                print aheader.type, aheader.contents
                ...
                if aheader.type == u'pos':
                    if not 'lang' in context:
                        ...
                        # so we assume it is the same language as the
                        # Wiktionary we're working on
                ...
                if aheader.contents == u'trans':
                    # Under the translations header there is quite a bit of
                    # stuff that's only needed for formatting, we can just skip
                    # that and go on processing the next line
                    lower = line.lower()
                    if ('{top}' in lower or
                            '{mid}' in lower or
                            '{bottom}' in lower or
                            ...
                            'here-->' in lower or
                            'width=' in lower or
                            '<!--left column' in lower or
                            '<!--right column' in lower):
                        continue
...
##            print contentblock['header']
            ...
            # Let's start by describing what to do with content found under
            # the POS header
...
##                            print part[:3], "Flag:", flag
                            if not flag and part[:3] == "'''":
                                sample = part.replace("'", '').strip()
##                                print 'Sample:', sample
                                # OK, so this should be an example of the term
                                # we are describing. Maybe it is necessary to
                                # compare it to the title of the page
                            if sample:
...
##                            print 'Gender: ',gender
                            ...
                            if part.replace("'", '')[:3].lower() == 'dim':
                                flag = 'diminutive'
                            if flag == 'plural':
                                plural = part.replace(',', '').replace(
                                    "'", '').strip()
##                                print 'Plural: ',plural
                            if flag == 'diminutive':
                                diminutive = part.replace(
                                    ',', '').replace("'", '').strip()
...
##                        print "contentblock['context'].['lang']",
##                        print contentblock['context']['lang']
                        if headercontent == 'noun':
                            theterm = term.Noun(
                                lang=contentblock['context']['lang'],
                                term=sample, gender=gender, number=number,
...
##                        raw_input("")
                    if line[:1].isdigit():
                        # Somebody didn't like automatic numbering and added
                        # static numbers of their own. Let's get rid of them
                        while line[:1].isdigit():
                            line = line[1:]
                        # and replace them with a hash, so the following if
                        # block picks it up
                        line = '#' + line
                    if line[:1] == "#":
                        # This probably is a definition
                        # If we already had a definition we need to store that
                        # one's data in a Meaning object and make that Meaning
                        # object part of the Page object
                        if definition:
                            ...
                            # comparative and superlative belong with the
                            # Adjective object conjugations belong with the
                            # Verb object

                            # Reset everything for the next round
                            sample = plural = diminutive = label = ''
                            definition = ''
                            examples = []

                            if not contentblock[
                                    'context']['lang'] in self.entries:
                                # If no entry for this language has been
                                # foreseen yet. Let's create one
                                anentry = entry.Entry(
                        ...
                        pos = line.find('<!--')
                        if pos != -1 and pos < 4:
                            # A html comment at the beginning of the line
                            # means this entry already has disambiguation
                            # labels, great
                            pos2 = line.find('-->')
...
##                        print "Example:", example
            ...
            if definition:
                ameaning = meaning.Meaning(term=theterm, definition=definition,
                                           label=label, examples=examples)
            ...
            # This is going to contain the Meaning object which has the
            # Definition which matches the Concisedef of the entry we are
            # working on right now:
            winner = False
            if headercontent == 'trans' or headercontent == 'syn' or \
               headercontent == 'ant':
                # On the English Wiktionary we will find concisedefs here to
                # link definitions to the content of these sections, but only
                ...
                for line in contentblock['text']:
                    if line[:3] == "'''":
                        ...
                        concisedefclean = concisedef.replace(
                    ...
                    if concisedefclean:
                        highest = 0
                        winner = anentry.meanings[
                            contentblock['context']['pos']][0]
...
##                        print 'winner:', winner.definition, 'score:', highest
                        ...
                        if headercontent == 'trans':
                            # We have to find a way to read the rest of the
                            # lines until the next ConciseDef into a structure,
                            # that can be processed later on. In contrast to a
                            # list of synonyms where the synonyms are on the
                            # rest of the lines, translations are on the
                            # following lines. It's also possible that there
                            # is no concisedef and that the translation's block
                            # simpy starts... or that there are numbers instead
                            # of concisedefs.
                            pass
